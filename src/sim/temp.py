...
def heaters_online(state):
    ...
    if heat_needed_c > 1.0 or state.hab_temp_c < state.target_temp_c - 1.5:
        target_heaters_online =  max_heaters_online
    
    elif heat_needed_c > 0.6:
        target_heaters_online = 5
    
    elif heat_needed_c > 0.25:
        target_heaters_online = 3
    
    elif heat_needed_c > 0.05:
        target_heaters_online = 1
    ...
    #---------------switch to standby---------------♡ 
        ...
        for heater in new_heaters:
            if heaters_not_needed > 0 and heater["status"] == "online":
                if heater["type"] == "backup":
                    heater["status"] = "standby"
                    heaters_not_needed -= 1
                    heaters_online_count -= 1

        if heaters_not_needed > 0:
            for heater in new_heaters:
                if heaters_not_needed > 0 and heater["status"] == "online":
                    if heater["type"] == "primary":
                        heater["status"] = "standby"
...
def radiators_online(state):
    ...
    #---------how many radiators needed online---------♡ 
    if cooling_needed_c > 2.0:
    ...
    #---------------switch to standby---------------♡ 
        ...
        radiators_not_needed = radiators_online_count - target_online

        for rad in new_radiators:
            if radiators_not_needed > 0 and rad["status"] == "online":
                if rad["type"] == "backup":
                    rad["status"] = "standby"
                    radiators_not_needed -= 1
                    radiators_online_count -= 1

        if radiators_not_needed > 0:
            for rad in new_radiators:
                if radiators_not_needed > 0 and rad["status"] == "online":
                    if rad["type"] == "primary":
                        rad["status"] = "standby"
...
def run_thermal_control(state, crew_heat_kw, oga_heat_kw, amine_bed_heat_added_kw, light_heat_kw, wellness_light_heat_kw, gh_heat_kw, gh_led_heat_kw, gh_chx_heat_added_kw, chx_heat_added_kw, isru_water_heat_added_kw, isru_atm_heat_added_kw, dt_min, sunlight_amount):
    hours_per_step = dt_min / 60.0
    ...
    hab_heat_kw = crew_heat_kw + oga_heat_kw + amine_bed_heat_added_kw + light_heat_kw + wellness_light_heat_kw + chx_heat_added_kw + gh_heat_kw + gh_led_heat_kw + gh_chx_heat_added_kw + isru_water_heat_added_kw + isru_atm_heat_added_kw
    ...
    radiator_heat_rejection_kw = rad_heat_rejection_kw(thermal_state, mars_temp_k, new_radiators)
