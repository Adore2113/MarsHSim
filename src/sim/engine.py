...
from .power import run_system_power
...
# file for running each simulation timestep

#--------------------constants-----------------------♡
default_dt_min = 5

#--------------------conversion----------------------♡
def mol_to_kpa(state, mol):
    gas_constant = 8.314462618
    hab_temp_k = state.hab_temp_c + 273.15

    pressure_pa = (mol * gas_constant * hab_temp_k) / state.hab_vol_m3
...
def step(state: Habitat_State, dt_min: int = default_dt_min):
    ...
    water_updates, water_outputs = run_water_system(
        ...
        greenhouse_outputs.get("greenhouse_direct_make_up_kg", 0.0),
